...
$this.on('change',function(){setInterval(function(){$('#style_'+id+'').text('input[type*="range"]#'+idex+'::-webkit-slider-thumb:after{content:"'+$this.val()+'";}')},5)});}
...
function rangeFiller(){var range=$('[type*="range"]:not([class*="-discrete"])');range.each(function(){var $this=$(this);var factorMax=$this.attr('max')/100;var lvlMax=Math.floor($this.attr('value')/factorMax);var lvl=$this.attr('value');if($this.attr('max')>100){$this.css('background','-webkit-linear-gradient(left, currentColor '+lvlMax+'%, #B3B3B3 '+lvlMax+'%)')}
...
deploy();});function rangeSlider(){var s=document.createElement('style'),r=document.querySelector('input[type=range][class*="-discrete"]'),track_prefs=['webkit-slider-runnable','moz-range'];document.body.appendChild(s);var getTrackStyleStr=function(el,val,prefs){var str='',len=prefs.length;for(var i=0;i<len;i++){str+='.js input[type=range]:focus::-'+prefs[i]+'-track{background-size:'+val+'}'}
return str;};var getTipStyleStr=function(el,val){var str='.js input[type=range]:focus /deep/ #thumb:after{content:"'+
val+'"}';return str;};var getValStr=function(el,p,i){var min=el.min||0,perc=(el.max)?~~(100*(p-min)/(el.max-min)):p,val='20% 100%, '+perc+'% 100%';return val;};r.addEventListener('input',function(){s.textContent=getTrackStyleStr(this,getValStr(this,this.value),track_prefs);s.textContent+=getTipStyleStr(this,this.value);},false);}